...
function Ussd({ linkZoneController }) {
  ...
  function onChangeUssdValue(e) {
    // console.log('onchangeussd', event.target.value)
    setUssdValue(e.target.value);
  }
  ...
  async function handleUSSD() {
    ...
    if (res.SendState === 2) setResponseDetails(res.UssdContent);
    else if (res.SendState === 3)
      setResponseDetails(
        "Ha ocurrido un error, intente cambiando Modo de Red (3G o Auto) en las configuraciones."
      );
    else setResponseDetails("Ha ocurrido un error, intente otra vez.");
  ...
  return (
    <div className="rounded-lg w-72 p-4 bg-white shadow-lg dark:bg-gray-800 max-w-xs m-5">
      ...
      <ul>
        <li className="text-xs font-inter leading-normal flex items-center font-medium text-black dark:text-white py-4 border-t border-gray-300">
          <select
        ...
        <li className="text-xs font-inter leading-normal font-medium text-black dark:text-white py-4 border-t border-gray-300">
          <textarea
          ...
          <div className=" w-full">
            <input
              ...
              name="email"
              placeholder="Ingrese el código USSD"
              value={ussdValue}
              onChange={(e) => onChangeUssdValue(e)}
            />
            <div className="flex w-full">
              <button
                ...
                className={
                  ...
                  " py-2 px-4 mt-5 bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500 focus:ring-offset-indigo-200 text-white w-full transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2  rounded-lg "
                }
                onClick={() => handleUSSD()}
              ...
              <button
                type="button"
                className={
                  (loadingCancel ? "animate-pulse" : "") +
                  "ml-1 py-2 px-4 mt-5 bg-transparent hover:bg-red-600 text-red-400 hover:text-white hover:border-red-600 border-red-400 border-2  focus:ring-red-500 focus:ring-offset-red-200 w-full transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2  rounded-lg "
                }
                onClick={() => cancelUSSD()}
                disabled={loadingCancel || loading}
              >
